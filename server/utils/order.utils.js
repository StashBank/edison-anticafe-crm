...
class CostHelper {
  async cost(order) {
    const tariffId = order.product && order.product.tariffId;
    const orderProducts = order.products || [];
    const tariffs = orderProducts.map(
      p => Object.assign({}, p.product.tariff, { quantity: +p.quantity })
    );
    if (tariffId) {
      const tariff = await this.fetchTariffById(tariffId);
  ...
  costOnce(order, tariff) {
    const tariffTypeCode = tariff && tariff.type && tariff.type.code;
    if (tariffTypeCode === 'manually') {
      return order.manualCost || 0.00;
    }
